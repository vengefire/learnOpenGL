...
{
  ModelBaseDep::ModelBaseDep()
  {
    Init();
  }

  ModelBaseDep::ModelBaseDep(std::shared_ptr<shaders::ShaderProgram>shaderProgram) : shader_program_(std::move(shaderProgram))
  {
    Init();
  }
  ...
  std::vector<float> ModelBaseDep::get_vertices() const
  {
    return vertices_;
  }

  void ModelBaseDep::set_vertices(std::vector<core::VertexBase> vertices)
  {
    vertices_data_ = std::move(vertices);
    buffer_vertex_data();
  }

  void ModelBaseDep::set_indices(std::vector<unsigned int> indices)
  {
  ...
  void ModelBaseDep::draw_elements_or_arrays(std::vector<core::VertexBase>::size_type count) const
  {
    if (!indices_.empty())
    {
      glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, ebo_Id_);
  ...
  void ModelBaseDep::buffer_vertex_data()
  {
    glBindVertexArray(vao_Id_);
  ...
  void ModelBaseDep::Init()
  {
    glGenVertexArrays(1, &vao_Id_);
    glGenBuffers(1, &vbo_Id_);
  }

  void ModelBaseDep::handle_event(std::shared_ptr<event::ProcessInputEventData> pEventData)
  {
  ...
  void ModelBaseDep::set_texture_from_file(const std::string& textureFilePath, bool flip_vertically)
  {
